	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/report_appraise/select_page.jsp");
		...
		if ("update".equals(action)) { // 來自update_report_appraise_input.jsp的請求
			...
			try {
				...
				/***************************2.開始修改資料*****************************************/
				Report_AppraiseService report_appraiseSvc = new Report_AppraiseService();
				report_appraiseVO = report_appraiseSvc.updateReport_Appraise(report_no, review_no, mem_no, emp_no, report_date, report_con, reply_sts); // 呼叫Service內updateEmp的方法
        ...
        if ("insert".equals(action)) { // 來自addReport_Appraise.jsp的請求  
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("report_appraiseVO", report_appraiseVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/report_appraise/addReport_Appraise.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				/***************************2.開始新增資料***************************************/
				Report_AppraiseService report_appraiseSvc = new Report_AppraiseService();
				report_appraiseVO = report_appraiseSvc.addReport_Appraise(review_no, mem_no, emp_no, report_date, report_con); // 呼叫Service內addReport_Appraise的方法				
